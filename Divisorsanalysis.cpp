...
void soln()
{
    ...
    cint(n);
    int nodiv=1ll,pr=1ll,sum=1ll;
    vector<pii>str;
    while(n--)
    {
        cint(x);
        cint(k);
        nodiv=mod_mul(nodiv,(mod_add(k,1,mod)),mod);
        str.pb({x,k});
        sum=mod_mul(sum,mod_mul(binpow(x,k+1,mod)-1,invprime(x-1,mod),mod),mod);
    }
    for(auto it:str)
        pr=mod_mul(pr,binpow(it.ft,it.sc*nodiv/2,mod),mod);
