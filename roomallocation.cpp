...
void soln()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            cnt++;
            pq.push({-dday[i].ft.sc, cnt});
            ans[dday[i].sc] = cnt;
        }
        else
        {
            pii front = pq.top();
            if (-front.ft < dday[i].ft.ft)
            {
                pq.pop();
                pq.push({-dday[i].ft.sc, front.sc});
                ans[dday[i].sc] = front.sc;
            }
            else
            {
                cnt++;
